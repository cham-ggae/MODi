    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="max-w-md mx-auto dark:bg-gray-800">
        ...
        <div className="space-y-4">
          {/* 템플릿 선택 */}
          <div>
            <label className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2 block">카드 템플릿</label>
          ...
          {/* 미리보기 */}
          <div>
            <label className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2 block">미리보기</label>
              ...
              <CardContent className="p-4">
                <div className="flex items-center gap-2 mb-2">
                  {(() => {
                    ...
                    return <Icon className={`w-5 h-5 ${template?.color.split(" ")[1] || "text-pink-600"}`} />
                  })()}
                  <h3 className="font-semibold text-gray-900 dark:text-white">{title || "제목을 입력하세요"}</h3>
                </div>
                <p className="text-gray-600 dark:text-gray-300 text-sm">{content || "내용을 입력하세요"}</p>
              </CardContent>
            </Card>
          </div>
